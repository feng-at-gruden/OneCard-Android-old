...
public class MCReader {
    ...
    public String[] readSector(int sectorIndex, byte[] key,
        ...
        useAsKeyB = false;
        key = KeyUtils.getkey(Common.getUID());
        ...
        if (auth) {
            ...
            for (int i = firstBlock; i < lastBlock; i++) {
                try {
                    ...
                    blocks.add(KeyUtils.byte2HexString(blockBytes));
                } catch (TagLostException e) {
                    throw e;
                } catch (IOException e) {
            ...
            if (!useAsKeyB) {
                if (isKeyBReadable(KeyUtils.hexStringToByteArray(
                        ret[last].substring(12, 20)))) {
                    ret[last] = KeyUtils.byte2HexString(key)
    ...
    public int buildNextKeyMapPart() {
        ...
        if (mKeysWithOrder != null && mLastSector != -1) {
            ...
            for (int i = 0; i < mKeysWithOrder.size();) {
                ...
                byte []key = KeyUtils.getkey(Common.getUID());
                try {
                    if (!foundKeys[0] &&
            ...
            if (lines != null) {
                for (String line : lines) {
                    if (!line.equals("") && line.length() == 12
                        ...
                        try {
                            keys.add(KeyUtils.hexStringToByteArray(line));
