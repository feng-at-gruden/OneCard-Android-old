...
import de.syss.MifareClassicTool.Utils.Constant;
import de.syss.MifareClassicTool.Utils.KeyUtils;
import de.syss.MifareClassicTool.Utils.MySocketThread;
...
public class Common extends Application {
     ...
     * (on external storage).
     */
    public static final String HOME_DIR = "/eHotel";
    ...
    /**
     * The latest card data
     */
    private static String mCardData = null;

    public static String getmCardData() {
        return mCardData;
    }

    public static void setmCardData(String mCardData) {
        Common.mCardData = mCardData;
    }
...
// ############################################################################
    ...
    public static int treatAsNewTag(Intent intent, Context context) {
        ...
        if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(intent.getAction())) {
            ...
            //TODO, BY FENG
            String msg = context.getResources().getString(R.string.info_new_tag_found) + " UID:" + KeyUtils.byte2HexString(tag.getId());

            byte[] key = KeyUtils.getkey(getUID());
            //byte[] key ={(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF,(byte)0xFF};
            String keyStr = KeyUtils.byte2HexString(key);
            msg += "\r\nKey:" + keyStr;

            final MCReader mReader = Common.checkForTagAndCreateReader(context);
            try {
                //String[] data = mReader.readSector(1, key, false);
                String[] data = mReader.readSector(2, key, false);
                String dataStr = "";
                String dataStr2 = "";
                ...
                setmCardData(dataStr2);

                Message keyMsg = new Message();
                keyMsg.what = MySocketThread.GetKeyData;
                keyMsg.obj = dataStr2;
                getMessageHandler.sendMessage(keyMsg);

                //Send data to server
                // sendMessage(dataStr2 + "\r\n");
                sendMessage(Common.TAG_BYCARD + dataStr2 + Common.TAG_BYCARD);
            } catch (Exception e) {
                e.printStackTrace();
            }
            if(mReader!=null)
    ...
    public static String readAsNewTag(Intent intent, Context context) {
        // Check if Intent has a NFC Tag.
        if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(intent.getAction())) {
            Tag tag = intent.getParcelableExtra(NfcAdapter.EXTRA_TAG);
            tag = MCReader.patchTag(tag);
    ...
    public static void setGetMessageHandler(Handler getMessageHandler) {
        Common.getMessageHandler = getMessageHandler;
    }
    ...
    public static void sendMessage(String msg) {
        if (mySocketThread.isConnected) {
            // mySocketThread.sendMessage(msg + "\r\n");
            mySocketThread.sendMessage(msg);
        } else {
            Toast.makeText(mAppContext, "Lost connection from server, reconnecting.", Toast.LENGTH_SHORT).show();
            initSocket();
        }
    }
    ...
    public static boolean isValueBlock(String hexString) {
        byte[] b = KeyUtils.hexStringToByteArray(hexString);
        if (b.length == 16) {
