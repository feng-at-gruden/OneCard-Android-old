...
public class MainMenu extends Activity {
    ...
    private TextView tagDataView, cardInfoView;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        tagDataView = (TextView) findViewById(R.id.tagDataView);
        cardInfoView = (TextView) findViewById(R.id.cardInfoView);
        tagDataView.setText("");
        cardInfoView.setText("");
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MySocketThread.ServerData:
                    ...
                    if( content.indexOf(Common.REC_MSG) >=0 ) {
                        showMessageDialog(content.replace(Common.REC_MSG, ""), null);
                    }else if( content.indexOf(Common.REC_BOOK_SUCCESS) >=0 ){
                        showMessageDialog(getString(R.string.message_book_success), null);
                    }else if( content.indexOf(Common.REC_INVALID_CARD_OR_ROOM) >=0 ){
                        showMessageDialog(getString(R.string.message_invalid_input, content), null);
                    }else if( content.indexOf(Common.REC_OUT_OF_SERVICE) >=0 ){
                        showMessageDialog(getString(R.string.message_out_of_service, content), null);
                    }else if( content.indexOf(Common.REC_BOOK_ERROR) >=0 ){
                ...
                case MySocketThread.SocketException:
                    Toast.makeText(MainMenu.this, getString(R.string.message_network_error, msg.obj.toString()), Toast.LENGTH_SHORT).show();
                    cardInfoView.setText("");
                    //Common.initSocket();
                    break;
    ...
    public void onResume() {
        ...
        if (mResume) {
            checkNfc();
            String content = Common.readAsNewTag(getIntent(), this);
            if(content != null)
                Common.sendShortMessage(Common.TAG_BYCARD + content + Common.TAG_BYCARD, getMessageHandler);
        }
        //Common.setGetMessageHandler(getMessageHandler);
        //Common.initSocket();
    }
    ...
    public void onNewIntent(Intent intent) {
        ...
        String content = Common.readAsNewTag(intent, this);
        if(content != null)
            Common.sendShortMessage(Common.TAG_BYCARD + content + Common.TAG_BYCARD, getMessageHandler);
    }
    ...
    private void showRoomInfoDialog(String content)
    {
        ...
        //TODO, check total content?
        v.findViewById(R.id.btnRegister).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Common.sendShortMessage(Common.CMD_REGISTRATION_BYCARD, getMessageHandler);
                closeRoomInfoDialog();
            }
        });
    ...
    private void showEnterRoomNumberDialog()
    {
        ...
        roomNumberEditText = (EditText) v.findViewById(R.id.roomNumber);
